...
// Hook personalizado para manejar listas
...
export function useLists(options: UseListsOptions = {}) {
  ...
  const getListWithTasks = useCallback(async (id: string) => {
    try {
      setLoading(true)
      setError(null)
      ...
      setError(err instanceof Error ? err : new Error("Error al cargar la lista"))
  ...
  const updateList = useCallback(async (id: string, updates: Partial<TaskList>) => {
    try {
      ...
      setLists((prevLists) => prevLists.map((list) => (list.id === id ? response.data : list)))
      return response.data
    } catch (err) {
  ...
  // Cargar listas automáticamente si autoFetch es true
  useEffect(() => {
